...
class ConnectFourEnv:
    ...
    def __init__(self, size: tuple = (6, 7), reward_setting={'step': 1, 'win': 100, 'draw': 10, 'loose': -100}, compressed_obs_representation=False):
        ...
        if not compressed_obs_representation:
            self.grid2obs = self.grid2obs_2grids
        else:
            self.grid2obs = self.grid2obs_1grid
    ...
    def grid2obs_1grid(self):
        '''
        Return the grid as an observation. Differently to original method, the grid isn't separated into one for each player

        :returns (tf.Tensor): An non-binary observation of shape (x, y) representing the board with 0 for no checker, 1 for player 1, 2 for player 2.
        '''
        # (A 2 in the grid represents the player who has opened the game.)
